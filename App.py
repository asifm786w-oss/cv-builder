...
def section_personal_details():
    ...
    if st.button("Improve professional summary (AI)", key="btn_improve_summary"):
        dbg_capture_state("before: improve_summary")
        ...
        if not spend_ai_credit(email_for_usage, source=f"ai_summary_improve:{int(time.time())}", amount=1):
            st.warning("You don’t have enough AI credits for this action.")
            st.stop()

        with st.spinner("Improving your professional summary..."):
            improved = generate_tailored_summary(
                {
...
def section_skills() -> list[str]:
    ...
    if st.button("Improve skills (AI)", key="btn_improve_skills"):
        dbg_capture_state("before: improve_skills")
        ...
        if not spend_ai_credit(email_for_usage, source="ai_skills_improve", amount=1):
            st.warning("You don’t have enough AI credits for this action.")
            st.stop()

        with st.spinner("Improving your skills..."):
            improved = improve_skills(skills_text)
            improved_limited = enforce_word_limit(improved, MAX_DOC_WORDS, label="Skills (AI)")
            stage_value("skills_text", improved_limited)

        dbg_diff_since_last("after: improve_skills")
...
def section_experience() -> list:
    ...
    # Only adjust count ONCE after CV autofill
    ...
    for i in range(int(num_experiences)):
        ...
        for k in [job_title_key, company_key, loc_key, start_key, end_key, desc_key]:
            safe_init_key(k, "")
        ...
        st.text_area("Description / key achievements", key=desc_key, help="Use one bullet per line.")

        if st.button("Improve this role (AI)", key=f"btn_role_ai_{i}"):
            dbg_capture_state(f"before: improve_role_{i}")
            ...
            cur = (st.session_state.get(desc_key) or "").strip()
            if not cur:
            ...
            if not spend_ai_credit(email_for_usage, source=f"ai_role_improve_{i+1}", amount=1):
                st.warning("You don’t have enough AI credits for this action.")
                st.stop()

            with st.spinner(f"Improving Role {i+1} description..."):
                improved = improve_bullets(cur)
            ...
            dbg_diff_since_last(f"after: improve_role_{i}")
            st.success(f"Role {i+1} updated.")
            st.rerun()

        # build Experience objects if complete
...
def section_education_and_references() -> tuple[list, str]:
    st.header("4. Education (multiple entries)")
    st.session_state.setdefault("num_education", 1)
    ...
    for i in range(int(num_education)):
        ...
        for k in [degree_key, institution_key, edu_location_key, edu_start_key, edu_end_key]:
            safe_init_key(k, "")
    ...
    references = st.text_area(
        ...
        key="references",
        help="Example: 'Available on request' or list names, roles and contact details.",
    )

    return education_items, references

def section_target_job_and_outputs(skills: list[str], experiences: list, education_items: list, references: str):
    st.header("5. Target Job (optional)")

    safe_init_key("job_description", "")
    apply_staged_value("job_description")
    ...
    if job_summary_clicked:
        dbg_capture_state("before: job_summary")
        ...
        if not try_spend(uid, source="job_summary", ai=1):
        ...
        dbg_diff_since_last("after: job_summary")
    ...
    if ai_cover_letter_clicked:
        dbg_capture_state("before: cover_letter")
        ...
        if not try_spend(uid, source="cover_letter", ai=1):
            st.warning("You don’t have enough AI credits to generate a cover letter.")
            st.stop()

        with st.spinner("Generating cover letter..."):
            cover_input = {
                "full_name": st.session_state.get("cv_full_name", ""),
                "current_title": st.session_state.get("cv_title", ""),
                "skills": skills,
        ...
        dbg_diff_since_last("after: cover_letter")
    ...
    if st.session_state.get("cover_letter"):
        ...
        edited = st.text_area("You can edit this before using it:", key="cover_letter_box", height=260)
    ...
    # Generate CV (does NOT clear widgets)
    ...
    if generate_clicked:
        dbg_capture_state("before: generate_cv")

        # IMPORTANT: do NOT clear widget keys here
        # clear_ai_upload_state_only() must be outputs-only; keep if it is safe.
        ...
        if not try_spend(uid, source="cv_generate", cv=1):
            st.warning("You don’t have enough CV credits to generate a CV.")
            st.stop()

        cv_full_name = (st.session_state.get("cv_full_name") or "").strip()
        cv_email = (st.session_state.get("cv_email") or "").strip()
        if not cv_full_name or not cv_email:
            st.error("Please fill in at least your full name and email.")
            st.stop()

        cv_summary = enforce_word_limit(st.session_state.get("cv_summary", "") or "", MAX_DOC_WORDS, "Professional summary")
        template_name = TEMPLATE_MAP.get(st.session_state.get("template_label"), "Blue Theme.html")

        cv = CV(
            full_name=cv_full_name,
            title=(st.session_state.get("cv_title") or None),
            email=cv_email,
            phone=(st.session_state.get("cv_phone") or None),
            full_address=None,
            location=(st.session_state.get("cv_location") or None),
        ...
        dbg_diff_since_last("after: generate_cv")
        ...
        with c2:
            st.download_button("📝 Download CV as Word (.docx)", data=docx_bytes, file_name="cv.docx",
