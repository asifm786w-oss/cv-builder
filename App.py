...
def auth_ui():
    ...
    with tab_register:
        ...
        if st.button("Create account", key="auth_btn_register"):
            ...
            if referral_code:
                try:
                    apply_referral_bonus(new_user_email=reg_email_n, referral_code=referral_code)
                except Exception as e:
            ...
            # store pending + hint user (tab UI won't auto-switch reliably)
            st.session_state["pending_verify_email"] = reg_email_n
            st.success("Account created. Now open the **Verify email** tab and enter the 6-digit code.")
            st.stop()
...
# If JD changed, clear ONLY explicit AI outputs (and the current epoch editor key)
if last_jd_fp and jd_fp != last_jd_fp:
    ...
    # epoch-safe editor key for current epoch
...
if ai_cover_letter_clicked:
    ...
    with st.spinner("Generating cover letter..."):
        try:
            ...
            # seed epoch editor key immediately
