...
# -------------------------------------------------------------------
# 2. Cover letter (V2): uses job summary + prevents role merging + mentions education
# -------------------------------------------------------------------
...
Education rule:
- If education exists, include ONE sentence referencing the most relevant / highest qualification (e.g., HND).
...
"""
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You write UK-style cover letters. You do not invent facts. You keep roles separate."},
            {"role": "user", "content": prompt},
        ],
        temperature=0.35,
    )
